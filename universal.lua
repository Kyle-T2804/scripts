-- safe loadstring wrapper avoiding recursion in environments without loadstring
local __raw_loadstring = (getgenv and getgenv().loadstring) or loadstring
local loadstring = function(...)
	local res, err
	if __raw_loadstring then
		res, err = __raw_loadstring(...)
	else
		err = 'loadstring not available in this environment'
	end
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		-- Apply Include/Exclude Teams from Settings > Modules
		local inc = vape.Categories.Main.Options['Include Teams']
		local exc = vape.Categories.Main.Options['Exclude Teams']
		local teamName = ent.Player and ent.Player.Team and ent.Player.Team.Name
		if teamName then
			if inc and #inc.ListEnabled > 0 and not table.find(inc.ListEnabled, teamName) then
				return false
			end
			if exc and table.find(exc.ListEnabled, teamName) then
				return false
			end
		end
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			...
			if whitelist.textdata ~= whitelist.olddata then
				local ann = whitelist.data and whitelist.data.Announcement
				if ann and ann.expiretime and ann.text then
					if tonumber(ann.expiretime) and tonumber(ann.expiretime) > os.time() then
						local targets = ann.targets
						targets = targets == 'all' and {tostring(lplr.UserId)} or type(targets) == 'string' and targets:split(',') or {}
						if table.find(targets, tostring(lplr.UserId)) then
							local hint = Instance.new('Hint')
							hint.Text = 'VAPE ANNOUNCEMENT: '..tostring(ann.text)
...
run(function()
		...
		Function = function(callback)
			...
			if callback then
				...
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					if CircleObject then
						CircleObject.Position = (AlwaysMiddle and AlwaysMiddle.Enabled) and (vape.gui.AbsoluteSize / 2) or inputService:GetMouseLocation()
					end
	...
	AimAssist:CreateToggle({
		...
		Function = function(callback)
			...
			if AlwaysMiddle and AlwaysMiddle.Object then
				AlwaysMiddle.Object.Visible = callback
			end
		end
	})
	...
	AlwaysMiddle = AimAssist:CreateToggle({
		Name = 'Always Middle',
		Function = function()
			if CircleObject then
				CircleObject.Position = (AlwaysMiddle.Enabled and (vape.gui.AbsoluteSize / 2)) or inputService:GetMouseLocation()
			end
		end,
		Darker = true,
		Visible = false
...
run(function()
			...
			if callback then
				if Method.Value == 'Ray' then
					oldray = hookfunction(Ray.new, function(origin, direction)
						...
						if calling then
							local list = #IgnoredScripts.ListEnabled > 0 and IgnoredScripts.ListEnabled or {'ControlScript', 'ControlModule', 'CameraModule', 'PlayerModule'}
	...
	AlwaysMiddle = SilentAim:CreateToggle({
		Name = 'Always Middle',
		Function = function()
			if CircleObject then
				CircleObject.Position = (AlwaysMiddle.Enabled and (vape.gui.AbsoluteSize / 2)) or inputService:GetMouseLocation()
...
run(function()
	...
	EntityUpkeep = vape.Categories.Utility:CreateModule({
		Name = 'Entity Upkeep',
		Function = function(callback)
			if callback then
				local running = true
				local th = task.spawn(function()
					repeat
						...
						-- granular wait so we can stop promptly on toggle/uninject
						local delay = tonumber(RefreshRate.Value) or 3
						local t0 = tick()
						while running and EntityUpkeep.Enabled and (tick() - t0) < delay do
							task.wait(math.clamp(delay / 20, 0.05, 0.5))
						end
					until not running or not EntityUpkeep.Enabled
				end)
				-- provide a Disconnect() for the maid system instead of storing a raw thread
				EntityUpkeep:Clean({
					Disconnect = function()
						running = false
		...
		Tooltip = 'Periodically refreshes entity list (fixes team changes, respawns, etc.)'
	})
	RefreshRate = EntityUpkeep:CreateSlider({
		Name = 'Refresh Rate',
		Min = 0.5,
		Max = 10,
		Decimal = 10,
...
-- (Team filter UI lives in Settings > Modules; no Utility module here)
